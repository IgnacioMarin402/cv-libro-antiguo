...
import { PAGE_DEPTH } from '../domain/pageCurl'
...
// rest of the chain curls inside it.
export default function Page({ number, page, opened, closedBook, materials, onClick, onPointerOver, onPointerOut }) {
  ...
  usePageCurl(group, skinnedMeshRef, opened, closedBook)

  return (
    <group ref={group} onClick={onClick} onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
      <primitive object={skinnedMesh} ref={skinnedMeshRef} position-z={-number * PAGE_DEPTH + page * PAGE_DEPTH} />
    </group>
  )
}
